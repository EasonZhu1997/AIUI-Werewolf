...
export class GlassesSpeech {
  ...
  async playSpeech(speech, kind) {
    ...
    try {
      const utterance = new this.Utterance(kind === 'narration' ? speech.text : `${speech.seat} 号，${speech.text}`);
      const voices = ['male-qn-jingying', 'female-yujie', 'male-qn-qingse', 'female-tianmei', 'Chinese (Mandarin)_Radio_Host', 'Chinese (Mandarin)_News_Anchor'];
      utterance.voice = kind === 'narration' ? 'Chinese (Mandarin)_Radio_Host' : voices[Math.max(0, Number(speech.seat) - 1) % voices.length];
      utterance.volume = 1;
