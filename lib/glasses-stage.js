...
export function glassesStage(view, { connected = true, listening = false, transcript = '', muted = false } = {}) {
  ...
  if (connected && view) {
    ...
    if (phase === 'lobby') {
      ...
      hint = prompt.kind === 'speech' ? listening ? '正在收音 · 说完停止并核对' : transcript ? '草稿未发送 · 核对后确认' : '主动开麦 · 核对文字后发送' : prompt.kind === 'vote' ? '左右选人或弃票 · 镜腿确认' : '按自己的夜间提示操作';
    } else if (phase === 'night') hint = '等待夜晚结束 · 暂无需操作';
